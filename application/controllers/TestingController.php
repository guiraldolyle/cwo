...
class Testingcontroller extends CI_Controller
{
    ...
    public function uploadLeasing()
    {
        $excelLeasing  = $_FILES['upload']['name'];
        $data = array();

        $file_name     = str_replace(['"', '[', ']'], '', json_encode($excelLeasing));
        $file_tmp_name = str_replace(['"', '[', ']'], '', json_encode($_FILES['upload']['tmp_name']));
        ...
        if ($file_name != '') {
            ...
            if ($ext == 'xlsx' || $ext == 'XLSX') {
                ...
                for ($x = 2; $x <= $highestRow; $x++) {
                    ...
                    $tradename     = trim($objWorksheet->getCellByColumnAndRow(0, $x)->getValue());
                    $email         = trim($objWorksheet->getCellByColumnAndRow(1, $x)->getValue());
                    $contactnumber = trim($objWorksheet->getCellByColumnAndRow(2, $x)->getValue());
                    $storeID       = trim($objWorksheet->getCellByColumnAndRow(3, $x)->getValue());

                    $query = $this->db->query('SELECT * FROM prospect WHERE store_id = "2" AND trade_name = "' . $tradename . '"')->ROW();
                    $data[] = ['trade_name' => $tradename, 'email' => $email, 'contact_number' => $contactnumber, 'store_id' => $storeID];

                    // var_dump($query);
                    $this->db->where('id', $query->id);
                    $this->db->update('prospect', ['email' => $email, 'contact_number' => $contactnumber]);
                }

                $msg = ['message' => 'Success', 'info' => 'Success'];
            } else {
    ...
    public function printSOP()
    {
        ...
        $supplier      = $this->sop_model->getData('SELECT s.supplier_code, s.supplier_name FROM sop_head sop INNER JOIN suppliers s ON s.supplier_id = sop.supplier_id WHERE sop.sop_id = 25 ');
        $customer      = $this->sop_model->getData('SELECT c.customer_code, c.customer_name FROM sop_head sop INNER JOIN customers c ON c.customer_code = sop.customer_code WHERE sop.sop_id =  25');
        $headData      = $this->sop_model->getHeadData(25);
        $invoiceData   = $this->sop_model->getInvoiceData(25);
        $deductionData = $this->sop_model->getDeductionData(25);
        $chargesData   = $this->sop_model->getChargesData(25);
