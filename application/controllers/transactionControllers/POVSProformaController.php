...
class Povsproformacontroller extends CI_Controller
{
            ...
            for ($i = 0; $i < count($_FILES['proforma']['name']); $i++) {
                ...
                if ($file_name != '') {
                    ...
                    if ($ext == 'xlsx' || $ext == 'XLSX') {
                        ...
                        $salesInvoiceNo      = trim($objWorksheet->getCellByColumnAndRow(1, 5)->getValue());
    ...
    public function getPendingMatches()
    {
        ...
        if (!empty($data)) {
            ...
            $this->db->trans_start();
            $result   = $this->povsproforma_model->getPendingMatches($data['supplier_id'], $data['customer_code']);
            $this->db->trans_complete();
